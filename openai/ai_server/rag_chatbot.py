...
chroma_client = chromadb.PersistentClient(
  path=CHROMA_PATH,
  settings = Settings(
    # 우리가 사용한 데이터를 크로마db 본사 서버에 익명으로 보낼건지(False=보내지않음)
    anonymized_telemetry=False, 
    # 명령어로 db 내용을 초기화 권한을 부여할건지 말건지. 배포할땐 False
    allow_reset=True)
)
...
def ingest_pdf(file_path:str, size : int):
  ...
  try:
    with open(file_path, "rb") as f:
      ...
      # 청킹(글자를을 분할)
      # size = 1000 # 한 분할에 포함될 문자 개수 
      # range(start, end, step) : start번지부터 end=1번지까지 step씩 증가
      # 리스트[start:end] / 문자열[start:end] : start번지부터 end-1번지까지 추출
      # ["0번지부터999번지까지로된몬자열", "1000~1999", ""]
...
print(rag_ask("해피 프라이데이의 조건이 어떻게 돼?")['message'])